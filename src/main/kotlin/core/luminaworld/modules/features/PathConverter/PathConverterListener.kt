...
class PathConverterListener(private val plugin: LuminaCore, private val module: PathConverterModule) : Listener {
    ...
    fun onPlayerInteract(event: PlayerInteractEvent) {
        ...
        // ทำการอัปเดตบล็อกใน Region Thread ของบล็อกนี้ เพื่อความปลอดภัยบน Folia
        plugin.server.regionScheduler.execute(plugin, blockLoc) {
            if (block.type != Material.DIRT_PATH) return@execute

            block.type = targetMaterial

            // ลดความทนทานของพลั่ว (คิดผลของยศ Unbreaking)
            ...
            val msg = config.getString("messages.converted", "%prefix% &aแปลงบล็อกทางเดินดินกลับเป็นบล็อกดินเรียบร้อย!") ?: ""
            if (msg.isNotEmpty()) {
                // ส่งผ่าน scheduler ของผู้เล่น

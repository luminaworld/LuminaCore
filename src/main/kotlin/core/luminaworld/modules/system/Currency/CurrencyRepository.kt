...
class CurrencyRepository(private val plugin: LuminaCore) {
        ...
        db?.runAsync { conn ->
            try {
                conn.createStatement().use { statement ->
                    statement.execute("""CREATE TABLE IF NOT EXISTS lumina_currency_balances (
                    ...
                    statement.execute("""CREATE TABLE IF NOT EXISTS lumina_currency_transactions (
                    ...
                    statement.execute("CREATE INDEX IF NOT EXISTS idx_lumina_currency_top ON lumina_currency_balances(currency_id, amount_minor)")
                }
    ...
    fun change(player: CurrencyPlayer, currency: String, amount: Long, operation: String, actor: UUID?, reason: String?, callback: (CurrencyOperationResult) -> Unit) = db?.runAsync { conn ->
        synchronized(conn) {
            try {
                ...
                if (new < 0) { callback(CurrencyOperationResult(CurrencyResult.INSUFFICIENT_FUNDS, old)); return@runAsync }
    ...
    fun transfer(from: CurrencyPlayer, to: CurrencyPlayer, currency: String, amount: Long, callback: (CurrencyOperationResult, Long) -> Unit) = db?.runAsync { conn ->
        synchronized(conn) {
            val oldAutoCommit = conn.autoCommit
            try {
                conn.autoCommit = false
                val senderBalance = readBalance(conn, from.uuid, currency)
                if (senderBalance < amount) { conn.rollback(); callback(CurrencyOperationResult(CurrencyResult.INSUFFICIENT_FUNDS, senderBalance), 0); return@runAsync }
                val receiverBalance = readBalance(conn, to.uuid, currency)
                writeBalance(conn, from, currency, senderBalance - amount)
                writeBalance(conn, to, currency, Math.addExact(receiverBalance, amount))
                log(conn, currency, "TRANSFER", from.uuid, from.uuid, to.uuid, amount, "player-transfer")
                conn.commit()
                callback(CurrencyOperationResult(CurrencyResult.SUCCESS, senderBalance - amount), receiverBalance + amount)
            } catch (e: Exception) {
    ...
    fun top(currency: String, limit: Int, callback: (List<CurrencyLeaderboardEntry>) -> Unit) = db?.runAsync { conn ->
        ...
        try {
            conn.prepareStatement("SELECT player_name, amount_minor FROM lumina_currency_balances WHERE currency_id = ? ORDER BY amount_minor DESC, player_name ASC LIMIT ?").use { st ->
    ...
    private fun readBalance(conn: Connection, player: UUID, currency: String): Long {
        conn.prepareStatement("SELECT amount_minor FROM lumina_currency_balances WHERE currency_id = ? AND player_uuid = ?").use { st ->
    ...
    private fun writeBalance(conn: Connection, player: CurrencyPlayer, currency: String, amount: Long) {
        val exists = conn.prepareStatement("SELECT 1 FROM lumina_currency_balances WHERE currency_id = ? AND player_uuid = ?").use { st ->
            st.setString(1, currency); st.setString(2, player.uuid.toString()); st.executeQuery().use { it.next() }
        }
        val now = System.currentTimeMillis()
        val sql = if (exists) "UPDATE lumina_currency_balances SET player_name = ?, amount_minor = ?, updated_at = ? WHERE currency_id = ? AND player_uuid = ?" else
            "INSERT INTO lumina_currency_balances (player_name, amount_minor, updated_at, currency_id, player_uuid) VALUES (?, ?, ?, ?, ?)"
    ...
    private fun log(conn: Connection, currency: String, operation: String, actor: UUID?, from: UUID?, to: UUID?, amount: Long, reason: String?) {
        conn.prepareStatement("INSERT INTO lumina_currency_transactions (currency_id, operation, actor_uuid, from_uuid, to_uuid, amount_minor, reason, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").use { st ->
